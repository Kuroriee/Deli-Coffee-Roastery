...
import { formatRupiah, buildWhatsAppLink, buildProductMessage, brand } from "../../mock/mock";
...
const productImageFor = (p) => {
  // deterministic image based on category
  const map = {
    "arabica-specialty":
  ...
  // subtle variation for luwak / wine
  if (p.id === "as-luwak") return "https://images.unsplash.com/photo-1607681034540-2c46cc71896d?crop=entropy&cs=srgb&fm=jpg&q=85";
  if (p.id === "as-wine") return "https://images.unsplash.com/photo-1447933601403-0c6688de566e?crop=entropy&cs=srgb&fm=jpg&q=85";
...
const ProductCard = ({ product }) => {
  ...
  const waLink = buildWhatsAppLink(
    ...
    buildProductMessage(product, qty)
  );
  ...
  return (
    <article className="card-lift bg-[#FBF6EC] rounded-3xl border border-[#3B2412]/10 overflow-hidden flex flex-col">
        ...
        {product.process && (
          <div
            className={`absolute bottom-3 left-3 rounded-full px-3 py-1 text-[11px] font-semibold ${
              processColor[product.process] || "bg-[#3B2412] text-[#F6EFE4]"
            }`}
          >
            {product.process}
          </div>
      ...
      <div className="p-5 flex flex-col flex-1">
        <h3 className="font-serif-warm text-xl text-[#3B2412] leading-tight">
          {product.name}
        </h3>
        {product.desc && (
          <p className="mt-1 text-sm text-[#3B2412]/70 line-clamp-2">
            {product.desc}
          </p>
        )}

        <div className="mt-4 flex items-baseline gap-1">
          <span className="font-serif-warm text-2xl text-[#1B7A43]">
